  ...
  config = mkIf (darlings.enable && cfg.enable) {
    ...
    home-manager.users.tiebe = {
      ...
      home.file.".config/discord".source =
        if evictCfg.enable
        then config.lib.file.mkOutOfStoreSymlink "/persist${evictCfg.baseDir}/.config/discord"
        else config.lib.file.mkOutOfStoreSymlink "/persist/home/tiebe/.config/discord";
      ...
      home.activation.createDiscordPersistDirs = lib.hm.dag.entryBefore ["writeBoundary"] ''
        $DRY_RUN_CMD ${pkgs.coreutils}/bin/mkdir -p $VERBOSE_ARG \
          ${
          if evictCfg.enable
          then ''"/persist${evictCfg.baseDir}/.config/discord"''
          else ''"/persist/home/tiebe/.config/discord"''
        }
      '';
    };
  };
