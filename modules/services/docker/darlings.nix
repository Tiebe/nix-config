  ...
  config = mkIf (darlings.enable && cfg.enable) {
    ...
    home-manager.users.tiebe = {config, ...}: {
      xdg.dataFile."docker".source =
        if evictCfg.enable
        then config.lib.file.mkOutOfStoreSymlink "/persist${evictCfg.configDir}/local/share/docker"
        else config.lib.file.mkOutOfStoreSymlink "/persist/home/tiebe/.local/share/docker";
    };
  };
}
